...
func update_progress():
	...
	var percent: float = MetaProgression.save_data['meta_upgrade_currency'] / upgrade.exp_cost
	percent = min(percent, 1.0)
	progress_bar.value = percent
	purchase_button.disabled = percent < 1.0 || current_quantity == upgrade.max_quantity
	if is_max_quantity:
